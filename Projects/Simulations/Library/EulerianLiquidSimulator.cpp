...
void EulerianLiquidSimulator::drawLiquidSurface(Renderer& renderer) const
{
	...
	liquidSurfaceMesh.drawMesh(renderer, true /* render tri faces */, Vec3f(.5),
											false /* don't render tri normals */, Vec3f(0),
											false /* don't render tri vertices */, Vec3f(0),
											true /* render tri edges */, Vec3f(0,1,0));
}

void EulerianLiquidSimulator::drawLiquidVelocity(Renderer& renderer, Axis planeAxis, float planePosition, float length) const
{
	myLiquidVelocity.drawSamplePointVectors(renderer, planeAxis, planePosition, Vec3f(0), myLiquidVelocity.dx() * length);
}
...
void EulerianLiquidSimulator::drawSolidSurface(Renderer& renderer) const
{
	...
	solidSurfaceMesh.drawMesh(renderer, false /* don't render tri faces */, Vec3f(0),
										false /* don't render tri normals */, Vec3f(0),
										false /* don't render tri vertices */, Vec3f(0),
										true /* render tri edges */, Vec3f(.25));
}

void EulerianLiquidSimulator::drawSolidVelocity(Renderer& renderer, Axis planeAxis, float planePosition, float length) const
{
	mySolidVelocity.drawSamplePointVectors(renderer, planeAxis, planePosition, Vec3f(0,1,0), mySolidVelocity.dx() * length);
}
...
void EulerianLiquidSimulator::setSolidVelocity(const VectorGrid<float>& solidVelocity)
{
	for (int axis : {0, 1, 2})
	{
		tbb::parallel_for(tbb::blocked_range<int>(0, solidVelocity.grid(axis).voxelCount(), tbbLightGrainSize), [&](tbb::blocked_range<int> &range)
		{

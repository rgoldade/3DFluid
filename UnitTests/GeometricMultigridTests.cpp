...
using namespace FluidSim3D::GeometricMultigridOperators;
...
static void testBoundaryCells(const UniformGrid<CellLabels>& cellLabels, const VectorGrid<double>* boundaryWeights)
{
    if (boundaryWeights != nullptr)
    {
        ...
        ASSERT_TRUE(boundaryWeights->size(0)[2] == cellLabels.size()[2]);

        ASSERT_TRUE(boundaryWeights->size(1)[0] == cellLabels.size()[0]);
        ASSERT_TRUE(boundaryWeights->size(1)[1] == cellLabels.size()[1] + 1);
        ASSERT_TRUE(boundaryWeights->size(1)[2] == cellLabels.size()[2]);

        ASSERT_TRUE(boundaryWeights->size(2)[0] == cellLabels.size()[0]);
        ASSERT_TRUE(boundaryWeights->size(2)[1] == cellLabels.size()[1]);
        ASSERT_TRUE(boundaryWeights->size(2)[2] == cellLabels.size()[2] + 1);
    }
    ...
    EXPECT_TRUE(unitTestBoundaryCells(cellLabels, boundaryWeights));
}
...
std::pair<UniformGrid<CellLabels>, VectorGrid<double>> buildTestDomain(bool useComplexDomain, bool useSolidSphere, int gridSize)
{
    ...
    std::pair<Vec3i, int> mgSettings = buildExpandedDomain(grids.first, grids.second, baseDomainCellLabels, baseBoundaryWeights);
...
static void testExteriorCells(const UniformGrid<CellLabels>& cellLabels)
{
    ...
    EXPECT_TRUE(unitTestExteriorCells(cellLabels));
}
...
static void testCoarsening(const UniformGrid<CellLabels>& coarseCellLabels, const UniformGrid<CellLabels>& fineCellLabels)
{
    ...
    ASSERT_TRUE(coarseCellLabels.size()[2] % 2 == 0);
    
    ASSERT_TRUE(fineCellLabels.size()[0] % 2 == 0);
    ASSERT_TRUE(fineCellLabels.size()[1] % 2 == 0);
    ASSERT_TRUE(fineCellLabels.size()[2] % 2 == 0);

    forEachVoxelRange(Vec3i::Zero(), fineCellLabels.size(), [&](const Vec3i& fineCell)
    {
    ...
    forEachVoxelRange(Vec3i::Zero(), coarseCellLabels.size(), [&](const Vec3i& coarseCell)
    {
    ...
    EXPECT_TRUE(unitTestCoarsening(coarseCellLabels, fineCellLabels));
}
...
static void symmetryTest(bool useComplexDomain, bool useSolidSphere, int gridSize)
{
    ...
    int mgLevels;
    {
        ...
        std::pair<Vec3i, int> mgSettings = buildExpandedDomain(domainCellLabels, boundaryWeights, baseDomainCellLabels, baseBoundaryWeights);
    ...
    // Jacobi smoother symmetry test
    {
        ...
        VecVec3i boundaryCells = buildBoundaryCells(domainCellLabels, 3);
    ...
    // Test direct solve symmetry
    {
        ...
        {
            forEachVoxelRange(Vec3i::Zero(), domainCellLabels.size(), [&](const Vec3i& cell)
            {
                if (domainCellLabels(cell) == CellLabels::INTERIOR_CELL || domainCellLabels(cell) == CellLabels::BOUNDARY_CELL)
                {
    ...
    // Test single level correction
    {
        ...
        {
            forEachVoxelRange(Vec3i::Zero(), coarseDomainLabels.size(), [&](const Vec3i& cell)
            {
...

        {
            ...
            tbb::parallel_for(tbb::blocked_range<int>(0, coarseDomainLabels.voxelCount(), tbbLightGrainSize), [&](const tbb::blocked_range<int>& range) {
                for (int cellIndex = range.begin(); cellIndex != range.end(); ++cellIndex)
                {
                    Vec3i cell = coarseDomainLabels.unflatten(cellIndex);
...
static void smootherTest(bool useComplexDomain, bool useSolidSphere, int gridSize)
{
    ...
    Vec3i deltaPoint = (deltaPercent * Vec3d::Constant(gridSize)).cast<int>() + exteriorOffset;
    
    forEachVoxelRange(deltaPoint - Vec3i::Ones(), deltaPoint + Vec3i::Constant(2), [&](const Vec3i& cell)
    {
        if (domainCellLabels(cell) == CellLabels::INTERIOR_CELL || domainCellLabels(cell) == CellLabels::BOUNDARY_CELL)
        {

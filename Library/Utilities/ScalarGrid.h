...
namespace ScalarGridSettings
{
	...
	enum class SampleType { CENTER, XFACE, YFACE, ZFACE, XEDGE, YEDGE, ZEDGE, NODE };
}
...
public:
	...
	ScalarGrid(const Transform& xform, const Vec3i& size,
	...
	{
		float offset = isIndexSpace ? 1E-1 : 1E-1 * myXform.dx();
		T dTdx = interp(worldPoint + Vec3f(offset, 0., 0.), isIndexSpace) - interp(worldPoint - Vec3f(offset, 0., 0.), isIndexSpace);
		T dTdy = interp(worldPoint + Vec3f(0., offset, 0.), isIndexSpace) - interp(worldPoint - Vec3f(0., offset, 0.), isIndexSpace);
		T dTdz = interp(worldPoint + Vec3f(0., 0., offset), isIndexSpace) - interp(worldPoint - Vec3f(0., 0., offset), isIndexSpace);
	...
	void drawCellSamplePoint(Renderer& renderer, const Vec3i& cell, const Vec3f& colour = Vec3f(1, 0, 0), float sampleSize = 5.) const;

	void drawSupersampledValues(Renderer& renderer, const Vec3f& start, const Vec3f& end, const Vec3f& sampleRadius, int samples, float sampleSize = 5) const;
	void drawSupersampledValuesVolume(Renderer& renderer, float sampleRadius = .5, int samples = 5, float sampleSize = 1) const;
	...
	void drawSupersampledValuesPlane(Renderer& renderer, Axis planeAxis, float position, float sampleRadius = .5, int samples = 5, float sampleSize = 1) const;
	void drawSampleGradientsPlane(Renderer& renderer, Axis planeAxis, float position, const Vec3f& colour = Vec3f(.5), float length = .25) const;
...
T ScalarGrid<T>::interp(const Vec3f& samplePoint, bool isIndexSpace) const
{
	...
	case BorderType::ASSERT:
		for (int axis : {0, 1, 2})
			assert(indexPoint[axis] >= 0 && indexPoint[axis] <= float(this->mySize[0] - 1));
		break;
	}
...
void ScalarGrid<T>::drawGridCell(Renderer& renderer, const Vec3i& cell, const Vec3f& colour) const
{
	...
	const Vec3f edgeToNodeOffset[12][2] = { { Vec3f(0, 0, 0), Vec3f(1, 0, 0) }, // x-axis edges
											{ Vec3f(0, 0, 1), Vec3f(1, 0, 1) },
											{ Vec3f(0, 1, 0), Vec3f(1, 1, 0) },
											{ Vec3f(0, 1, 1), Vec3f(1, 1, 1) },

											{ Vec3f(0, 0, 0), Vec3f(0, 1, 0) }, // y-axis edges
											{ Vec3f(1, 0, 0), Vec3f(1, 1, 0) },
											{ Vec3f(0, 0, 1), Vec3f(0, 1, 1) },
											{ Vec3f(1, 0, 1), Vec3f(1, 1, 1) },

											{ Vec3f(0, 0, 0), Vec3f(0, 0, 1) }, // z-axis edges
											{ Vec3f(1, 0, 0), Vec3f(1, 0, 1) },
											{ Vec3f(0, 1, 0), Vec3f(0, 1, 1) },
...
template<typename T>
void ScalarGrid<T>::drawCellSamplePoint(Renderer& renderer, const Vec3i& cell, const Vec3f& colour, float sampleSize) const
{
	std::vector<Vec3f> samplePoints;
...
template<typename T>
void ScalarGrid<T>::drawSupersampledValuesVolume(Renderer& renderer, float sampleRadius, int samples, float sampleSize) const
{
	Vec3f start(0);
...
// Warning: there is no protection here for ASSERT border types
template<typename T>
void ScalarGrid<T>::drawSupersampledValuesPlane(Renderer& renderer, Axis planeAxis, float position,
...
template<typename T>
void ScalarGrid<T>::drawSampleGradientsPlane(Renderer& renderer, Axis planeAxis, float position,

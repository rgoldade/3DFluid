...
{
public:
    Vec<N, T>() { static_assert(N >= 0, "Vector dimensions must be positive"); }

    explicit Vec<N, T>(T val)
    {
        static_assert(N >= 0, "Vector dimensions must be positive");
        myVec.fill(val);
    }

    constexpr Vec<N, T>(T v0, T v1) : myVec({v0, v1}) { static_assert(N == 2, "Vector dimensions must match"); }
    ...
    {
        for (int i = 0; i < N; ++i) myVec[i] = static_cast<T>(source[i]);
    }

    T& operator[](int index) { return myVec[index]; }

    const T& operator[](int index) const { return myVec[index]; }
    ...
    {
        for (int i = 0; i < N; ++i) myVec[i] += rhs[i];
        return *this;
    }
    ...
    Vec<N, T> operator-() const  // unary minus
    {
        ...
        for (int i = 0; i < N; ++i) vec[i] = -myVec[i];
...
T mag2(const Vec<N, T>& vec)
{
    ...
    for (int i = 0; i < N; ++i) val += sqr(vec[i]);
...
T dist2(const Vec<N, T>& vec0, const Vec<N, T>& vec1)
{
    ...
    for (int i = 0; i < N; ++i) val += sqr(vec0[i] - vec1[i]);
...
T infNorm(const Vec<N, T>& vec)
{
    ...
    for (int i = 0; i < N; ++i) val = max(std::fabs(vec[i]), val);
...
{
    Vec<N, T> localVec;
    for (int i = 0; i < N; ++i) localVec[i] = (vec0[i] < vec1[i]) ? vec0[i] : vec1[i];
...
T dot(const Vec<N, T>& vec0, const Vec<N, T>& vec1)
{
    ...
    for (int i = 0; i < N; ++i) val += vec0[i] * vec1[i];
...
{
    return Vec<3, T>(vec0[1] * vec1[2] - vec0[2] * vec1[1], vec0[2] * vec1[0] - vec0[0] * vec1[2],
...
{
    Vec<N, T> localVec;
    for (int i = 0; i < N; ++i) localVec[i] = std::round(vec[i]);
...
{
    Vec<N, T> localVec;
    for (int i = 0; i < N; ++i) localVec[i] = std::floor(vec[i]);
...
{
    Vec<N, T> localVec;
    for (int i = 0; i < N; ++i) localVec[i] = std::ceil(vec[i]);
...
{
    Vec<N, T> localVec;
    for (int i = 0; i < N; ++i) localVec[i] = std::fabs(vec[i]);
...
{
    Vec<N, T> localVec;
    for (int i = 0; i < N; ++i) localVec[i] = clamp(vec[i], low[i], high[i]);
...
{
    Vec<N, T> localVec;
    for (int i = 0; i < N; ++i) localVec[i] = std::min(min0[i], min1[i]);
...
{
    Vec<N, T> localVec;
    for (int i = 0; i < N; ++i) localVec[i] = std::max(max0[i], max1[i]);
...
{
    for (int i = 0; i < N; ++i) minAndMax(vecMin[i], vecMax[i], vec0[i], vec1[i]);
}

template <int N, typename T>
void updateMinOrMax(Vec<N, T>& vecMin, Vec<N, T>& vecMax, const Vec<N, T>& vec)
{
    for (int i = 0; i < N; ++i) updateMinOrMax(vecMin[i], vecMax[i], vec[i]);
}

template <int N, typename T>
void updateMinAndMax(Vec<N, T>& vecMin, Vec<N, T>& vecMax, const Vec<N, T>& vec)
{
    for (int i = 0; i < N; ++i) updateMinAndMax(vecMin[i], vecMax[i], vec[i]);
}

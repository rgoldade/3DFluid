                             ...
                             tbb::enumerable_thread_specific<std::vector<StorageType>>& parallelVector)
{
    ...
    parallelVector.combine_each([&](const std::vector<StorageType>& localVector) { vectorSize += localVector.size(); });
...
inline double randhashd(unsigned seed) { return randhash(seed) / double(maxInteger); }

inline float randhashf(unsigned seed) { return randhash(seed) / float(maxInteger); }

// returns repeatable stateless pseudo-random number in [a,b]
inline double randhashd(unsigned seed, double a, double b) { return (b - a) * randhash(seed) / double(maxInteger) + a; }

inline float randhashf(unsigned seed, float a, float b) { return (b - a) * randhash(seed) / float(maxInteger) + a; }

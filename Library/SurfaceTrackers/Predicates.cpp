...
{
	// Exploit the fact that the edge is z-axis aligned and project
	// the triangle and the edge vertex down to the xy-plane. If the projection
	// of "a" is inside the triangle, then running orient2d on each triangle edge pair
	// vs. vertex "a" should yield the same result (CCW or CW winding). We have already
	// adjusted qrs to be CCW itself, so "a" should give CCW for each edge pair as well

	// If any of the three tests give CW winding, then the edge does not intersect the triangle
	REAL pqr = orient2d(a.data(), q.data(), r.data());
	if (pqr < 0) return false;
	REAL prs = orient2d(a.data(), r.data(), s.data());
	if (prs < 0) return false;
	REAL psq = orient2d(a.data(), s.data(), q.data());
	if (psq < 0) return false;

	// If "a" projection falls strictly inside the triangle, it easily intersects
	if (pqr > 0 && prs > 0 && psq > 0) return true;

	// By now we know there is an intersection and it is degenerate in one of a few different
	// ways. We need to handle the cases where the grid intersections the triangle edge,
	// the grid-edge intersects the triangle vertex and where the triangle intersects the grid node.

	// pqr equals zero and the other two edge tests are not. Therefore "a" strictly intersects edge pqr.
	// These conditions implicitly check if moving "a" to the right would create an intersection
	// and if (q,r) is horizontal, then would moving "a" up create intersection?
	// I think the x-check is redundant since the CCW winding order and non-degenerate triangle
	// rules have already been applied earlier
	...
	// Given the CCW winding order, this check implicitly shifts "a" positive in both the x-y directions
...
{
	// Re-orient so ray is pointing along the z-axis
	if (axis == Axis::XAXIS)
	{
		a = Vec3R(a[2], a[1], a[0]);
		q = Vec3R(q[2], q[1], q[0]);
	...
	// Adjust winding order of the triangle so that Vec3R(0,0,-inf) is "below" qrs
	// using the orient3d convention. By exploiting the fact that the edge we're
	// testing against is z-axis aligned, we can project qrs onto an xy-plane and test
	// the winding order -- this is done simply by only passing the x,y values
	// to orient2d
	REAL qrs = orient2d(q.data(), r.data(), s.data());

	if (qrs == 0)
		return IntersectionLabels::NO; // qrs and z-axis edge are co-planar so no intersection
	else if (qrs < 0) // re-order triangle winding so qrs is CCW
		std::swap(r, s);
	// else qrs > 0. and we don't need to swap anything

	bool isRayIntersecting = zCastIntersection(a, q, r, s);
	// If there is no intersection, we're done
	if (!isRayIntersecting) return IntersectionLabels::NO;

	// Check that grid point is below the triangle's plane.
	// If it's above, there is no intersection.
	// If it is on the plane, we need to decide if it should proceed.
	REAL pqrs = orient3d(a.data(), q.data(), r.data(), s.data());
	
	if (pqrs > 0) return IntersectionLabels::NO;
	else if (pqrs == 0) return IntersectionLabels::ON;
	else return IntersectionLabels::YES;
}

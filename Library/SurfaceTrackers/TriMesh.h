...
public:
	...
	void operator=(const Vec3f& point)
	{
		myPoint = point;
	}

	// Get triangle face indicent to vertex
	int triFace(int index) const
	{
		return myTriFaces[index];
	}
...
public:
	...
	bool findVertex(int index) const
	{
		if (myVertices[0] == index ||
			myVertices[1] == index ||
			myVertices[2] == index) return true;
	...
	void reverse()
	{
		std::swap(myVertices[0], myVertices[2]);
	}
...
public:
	...
	TriMesh(const std::vector<Vec3i>& triFaces, const std::vector<Vec3f>& vertices)
	{
	...
	const std::vector<TriFace>& triFaces() const
	{
		return myTriFaces;
	}

	const TriFace& triFace(int index) const
	{
		return myTriFaces[index];
	}
	...
	void setVertex(int index, const Vec3f& vertex)
	{
	...
	Vec3f normal(int triFaceIndex) const
	{
		return normalize(scaledNormal(triFaceIndex));
	}
	...
	void drawMesh(Renderer& renderer,
		bool doRenderTriFaces = false,
		Vec3f triFaceColour = Vec3f(0),
		bool doRenderTriNormals = false,
		Vec3f normalColour = Vec3f(0),
		bool doRenderVertices = false,
		Vec3f vertexColour = Vec3f(0),
		bool doRenderTriEdges = false,

...
void TriMesh::drawMesh(Renderer& renderer,
						bool doRenderTriFaces,
						Vec3f triFaceColour,
						bool doRenderTriNormals,
						Vec3f normalColour,
						bool doRenderVertices,
						Vec3f vertexColour,
						bool doRenderTriEdges,
						Vec3f edgeColour)
{
	...
	if (doRenderTriFaces)
		renderer.addTriFaces(vertexPoints, vertexNormals, triFaces, triFaceColour);
	...
	if (doRenderTriNormals)
	{
		...
		tbb::parallel_for(tbb::blocked_range<int>(0, myTriFaces.size(), tbbLightGrainSize), [&](const tbb::blocked_range<int>& range)
		{
			for (int triFaceIndex = range.begin(); triFaceIndex != range.end(); ++triFaceIndex)
			{
				...
				for (int localVertexIndex : {0, 1, 2})
					localNormalStart += myVertices[myTriFaces[triFaceIndex].vertex(localVertexIndex)].point();

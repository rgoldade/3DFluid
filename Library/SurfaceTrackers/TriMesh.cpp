...
{
    myTriFaces.reserve(triFaces.size());
    for (const auto& triFace : triFaces) myTriFaces.emplace_back(triFace);

    myVertices.reserve(vertices.size());
    for (const auto& vertex : vertices) myVertices.emplace_back(vertex);

    // Update vertices to store adjacent triangle faces
    for (int triIndex = 0; triIndex < myTriFaces.size(); ++triIndex)
    {
        for (int localVertexIndex : {0, 1, 2})
        {
            int meshVertexIndex = myTriFaces[triIndex].vertex(localVertexIndex);
            myVertices[meshVertexIndex].addTriFace(triIndex);
        }
    }
}
...
void TriMesh::insertMesh(const TriMesh& mesh)
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(vertexCount, myVertices.size(), tbbLightGrainSize),
                      [&](const tbb::blocked_range<int>& range) {
                          for (int vertexIndex = range.begin(); vertexIndex != range.end(); ++vertexIndex)
                              for (int neighbourTriFaceIndex = 0;
                                   neighbourTriFaceIndex < myVertices[vertexIndex].valence(); ++neighbourTriFaceIndex)
                              {
    ...
    tbb::parallel_for(tbb::blocked_range<int>(triFaceCount, myTriFaces.size(), tbbLightGrainSize),
                      [&](const tbb::blocked_range<int>& range) {
                          for (int triFaceIndex = range.begin(); triFaceIndex != range.end(); ++triFaceIndex)
                              for (int localVertexIndex : {0, 1, 2})
                              {
                                  ...
                                  myTriFaces[triFaceIndex].replaceVertex(meshVertexIndex,
...
std::vector<Vec3f> TriMesh::vertexNormals() const
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, myTriFaces.size(), tbbLightGrainSize),
                      [&](const tbb::blocked_range<int>& range) {
                          for (int triFaceIndex = range.begin(); triFaceIndex != range.end(); ++triFaceIndex)
                              triFaceWeightedNormals[triFaceIndex] = scaledNormal(triFaceIndex);
                      });
...
void TriMesh::drawMesh(Renderer& renderer, bool doRenderTriFaces, Vec3f triFaceColour, bool doRenderTriNormals,
                       Vec3f normalColour, bool doRenderVertices, Vec3f vertexColour, bool doRenderTriEdges,
                       Vec3f edgeColour)
{
        ...
        tbb::parallel_for(tbb::blocked_range<int>(0, myTriFaces.size(), tbbLightGrainSize),
                          [&](const tbb::blocked_range<int>& range) {
                              for (int triFaceIndex = range.begin(); triFaceIndex != range.end(); ++triFaceIndex)
                              {
                                  ...
                                  for (int localVertexIndex : {0, 1, 2})
                                      localNormalStart +=
                                          myVertices[myTriFaces[triFaceIndex].vertex(localVertexIndex)].point();
                                  ...
                                  endPoints[triFaceIndex] =
                                      startPoints[triFaceIndex] + .1 * normalize(weightedTriNormals[triFaceIndex]);
                              }
                          });
                          ...
                          [&](const tbb::blocked_range<int>& range) {
                              for (int triFaceIndex = range.begin(); triFaceIndex != range.end(); ++triFaceIndex)
                                  for (int localStartVertexIndex : {0, 1, 2})
                                  {
                                      ...
                                      startPoints[3 * triFaceIndex + localStartVertexIndex] =
                                          myVertices[myTriFaces[triFaceIndex].vertex(localStartVertexIndex)].point();
                                      endPoints[3 * triFaceIndex + localStartVertexIndex] =
                                          myVertices[myTriFaces[triFaceIndex].vertex(localEndVertexIndex)].point();
                                  }
                          });
...
bool TriMesh::unitTestMesh() const
{
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, myVertices.size(), tbbLightGrainSize),
                      [&](const tbb::blocked_range<int>& range) {
                          ...
                          for (int vertexIndex = range.begin(); vertexIndex != range.end(); ++vertexIndex)
                              for (int adjacentTriFaceIndex = 0;
                                   adjacentTriFaceIndex < myVertices[vertexIndex].valence(); ++adjacentTriFaceIndex)
                              {
                                  int triFaceIndex = myVertices[vertexIndex].triFace(adjacentTriFaceIndex);

                                  if (!myTriFaces[triFaceIndex].findVertex(vertexIndex))
                                      localFailedMeshPairs.emplace_back(vertexIndex, triFaceIndex);
                              }
                      });
    ...
    if (failedMeshPairs.size() > 0)
    {
        for (const auto& failedPair : failedMeshPairs)
            std::cout << "Unit test failed in adjacent tri-face test. Vertex: " << failedPair[0]
                      << ". Tri: " << failedPair[1] << std::endl;
                      ...
                      [&](const tbb::blocked_range<int>& range) {
                          ...
                          for (int triFaceIndex = range.begin(); triFaceIndex != range.end(); ++triFaceIndex)
                              for (int localVertexIndex : {0, 1, 2})
                              {
                                  ...
                                  if (!myVertices[meshVertexIndex].findTriFace(triFaceIndex))
                                      localFailedMeshPairs.emplace_back(meshVertexIndex, triFaceIndex);
                              }
                      });
    ...
    {
        for (const auto& failedPair : failedMeshPairs)
            std::cout << "Unit test failed in adjacent vertex test. Vertex: " << failedPair[0]
                      << ". Tri: " << failedPair[1] << std::endl;

	...
	, myInitialGuessPressure(nullptr)
{
	...
	myValidFaces = VectorGrid<VisitedCellLabels>(surface.xform(), surface.size(), Vec3t<VisitedCellLabels>::Constant(VisitedCellLabels::UNVISITED_CELL), VectorGridSettings::SampleType::STAGGERED);
}
	...
	bool useMGPreconditioner)
{
	...
	tbb::parallel_for(tbb::blocked_range<int>(0, mySurface.voxelCount()), [&](const tbb::blocked_range<int>& range)
	{
		...
		{
			Vec3i cell = mySurface.unflatten(cellIndex);
	...
	if (!hasDirichletCell)
	{
		Vec3d accumulatedDivergence = tbb::parallel_deterministic_reduce(tbb::blocked_range<int>(0, baseDomainCellLabels.voxelCount(), tbbLightGrainSize), Vec3d::Zero().eval(),
		[&](const tbb::blocked_range<int>& range, Vec3d divergenceStats) -> Vec3d
		{
			for (int cellIndex = range.begin(); cellIndex != range.end(); ++cellIndex)
			{
		...
		[](const Vec3d& stats0, const Vec3d& stats1) -> Vec3d
		{
		...
		forEachVoxelRange(Vec3i::Zero(), baseDomainCellLabels.size(), [&](const Vec3i& cell)
		{
	...
	VectorGrid<double> poissonFaceWeights(mySurface.xform(), mySurface.size(), Vec3d::Zero(), VectorGridSettings::SampleType::STAGGERED);
	...
	for (int axis : {0, 1, 2})
	{
		tbb::parallel_for(tbb::blocked_range<int>(0, poissonFaceWeights.grid(axis).voxelCount()), [&](const tbb::blocked_range<int>& range)
		{
			...
			{
				Vec3i face = poissonFaceWeights.grid(axis).unflatten(faceIndex);
	...
	std::pair<Vec3i, int> mgSettings = GeometricMultigridOperators::buildExpandedDomainLabels(mgDomainCellLabels, baseDomainCellLabels);

	Vec3i expandedOffset = mgSettings.first;
	int mgLevels = mgSettings.second;

	VectorGrid<double> mgBoundaryWeights(poissonFaceWeights.xform(), mgDomainCellLabels.size(), Vec3d::Zero(), VectorGridSettings::SampleType::STAGGERED);
	...
	tbb::parallel_for(tbb::blocked_range<int>(0, mgDomainCellLabels.voxelCount()), [&](const tbb::blocked_range<int>& range)
	{
		for (int cellIndex = range.begin(); cellIndex != range.end(); ++cellIndex)
		{
			Vec3i cell = mgDomainCellLabels.unflatten(cellIndex);
	...
	if (useMGPreconditioner)
	{
		solveGeometricConjugateGradient(mgSolutionGrid,
			...
			addToVector,
			addScaledVector,
			1E-6 /*solver tolerance*/, 1000 /* max iterations */, true);
	}
	else
	{
		solveGeometricConjugateGradient(mgSolutionGrid,

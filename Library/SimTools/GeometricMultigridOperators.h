#ifndef FLUIDSIM3D_GEOMETRIC_MULTIGRID_OPERATORS_H
#define FLUIDSIM3D_GEOMETRIC_MULTIGRID_OPERATORS_H
...
VecVec3i buildBoundaryCells(const UniformGrid<CellLabels>& sourceCellLabels, int boundaryWidth);
...
std::pair<Vec3i, int> buildCustomExpandedDomainLabels(UniformGrid<CellLabels>& expandedDomainCellLabels,
														...
														const IsInteriorFunctor& isInteriorFunctor)
{
	...
	Vec3i expandedGridSize = baseCustomLabels.size() + 2 * Vec3i::Constant(exteriorPadding);
	...
	tbb::parallel_for(tbb::blocked_range<int>(0, baseCustomLabels.voxelCount(), tbbLightGrainSize), [&](const tbb::blocked_range<int>& range)
	{
		...
		{
			Vec3i cell = baseCustomLabels.unflatten(cellIndex);
